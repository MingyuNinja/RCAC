...
def train(config, group, device="auto"):
    # Create the save path and save the config
    print("[research] Training agent with config:")
    print(config)
    path = f'./logs/{config["env"]}_{group}_{random.randint(int(1e5), int(1e6) - 1)}'
    ...
    if use_wandb:
        ...
        import wandb
        project_dir = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
        wandb.init(project='XQL-Interpolate-Online', 
                   group=f'{config["env"]}_{group}',
                   name=os.path.basename(path), 
                   config=config.flatten(separator="-"),
                   dir=os.path.join(os.path.dirname(project_dir), "wandb"))
