...
# Add dropout / layernorm / batchnorm?
class MLP(nn.Module):

    def __init__(self, input_dim, output_dim, hidden_layers=[256, 256], act=nn.ReLU, output_act=None, ln=False, dropout=None):
        super().__init__()
        net = []
        last_dim = input_dim
        for dim in hidden_layers:
            if ln: net.append(nn.LayerNorm(last_dim))
            net.append(nn.Linear(last_dim, dim))
            if dropout is not None: net.append(nn.Dropout(p=dropout))
            net.append(act())
            last_dim = dim
...
class EnsembleMLP(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, ensemble_size=3, hidden_layers=[256, 256], act=nn.ReLU, output_act=None, ln=False, dropout=None):
        '''
        An ensemble MLP
