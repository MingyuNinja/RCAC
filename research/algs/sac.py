...
class SAC(Algorithm):
    ...
    def setup_optimizers(self, optim_class, optim_kwargs):
        ...
        if self._alpha is None: # Manual alpha
    ...
    def _update_actor_and_alpha(self, batch):
        ...
        if self.offline:
            ...
            qs_mu = self.network.critic(obs, mu)
            q_mu = torch.min(qs_mu, dim=0)[0]
            
            exp_a = torch.exp((q_mu - q) / self.alpha)
            
            lmbda = 0.25/q.abs().mean().detach()
            # actor_loss = -(exp_a * log_prob_mu).mean()
            actor_loss = torch.clamp(self.alpha.detach() * log_prob - q, min=-4).mean() - log_prob_mu.mean()
        ...
        if self._alpha is None:
            ...
            alpha_loss = entropy # Dummy value
    ...
    def _train_step(self, batch):
        ...
        if self.steps % self.reset_freq == 0:
