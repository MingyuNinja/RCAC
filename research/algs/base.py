...
class Algorithm(ABC):
    ...
    def train(self, path, total_steps, schedule=None, schedule_kwargs={}, 
        ...
        # Check Offline -> Is this error source?
        try:
            offline = d4rl.qlearning_dataset(self.env) # Check offline here!
            print(f'[research] Using offline dataset : {self.env.unwrapped.spec.id}')
            self.offline = True
            self.dataset.offline = True
            self.dataset.setup()
            self.dataset.load_offline(offline)
        except:
            print(f'[research] Using online env : {self.env.unwrapped.spec.id}')
        
        # Setup training
        if not self.offline: self._setup_train()
        self.network.train()
            ...
            for batch in dataloader:
                ...
                if (current_step - last_validation_log) >= eval_freq:
                    ...
                    if self.eval_env is not None and eval_ep > 0:
                        eval_metrics = eval_policy(self.eval_env, self, eval_ep, discount=self.dataset.discount)
