...
class D4(Algorithm):
    ...
    def _update_actor(self, batch):
        ...
        action_target = self.target_network.actor(obs).detach() # target policy action
        action_clamp = torch.clamp(action, 
            min=action_target-action*self.max_action, 
            max=action_target+action*self.max_action)
        clamp_qs = self.target_network.critic(obs, action_clamp)
        clamp_q = target_qs[0] # Take only the first Q function
        
        actor_loss = -torch.min(q, clamp_q).mean() # Deterministic policy
