...
class REDQ(Algorithm):
    ...
    def __init__(self, env, network_class, dataset_class, 
        ...
        self.num_q_to_use = num_q_to_use
        self.num_q_fns = self.network.critic.num_q_fns
    ...
    def _update_critic(self, batch):
        # Choose Q-function to update
        sample_idxs = np.random.choice(self.num_q_fns, self.num_q_to_use, replace=False)
        
        with torch.no_grad():
            dist = self.network.actor(batch['next_obs'])
            next_action = dist.rsample()
            log_prob = dist.log_prob(next_action).sum(dim=-1)
            target_qs = self.target_network.critic(batch['next_obs'], next_action)[sample_idxs]
        ...
        q_loss = sum([torch.nn.functional.mse_loss(qs[i], target_q) for i in sample_idxs])
    ...
    def _update_actor_and_alpha(self, batch):
        ...
        q = torch.mean(qs, dim=0)[0] # Average every ensemble of Q-functions
    ...
    def _train_step(self, batch):
        ...
        if self.reset_freq is not None and self.steps % self.reset_freq == 0:
